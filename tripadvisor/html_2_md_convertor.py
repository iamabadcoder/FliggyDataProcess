...
def get_element_list(allPages):
	...
	while idx < length:
		...
		if (get_rgb(ele['color']) > 150 or ele['font-size'] > 40) and (
						ele['zzz'].startswith('问') or re.compile('D[1-9].*').match(ele['zzz']) or re.compile(
					'No\.').match(ele['zzz'])):
...
def joint_equal(ele_now, ele_pre):
	# todo check contents
	if int(ele_now['bottom'] - ele_pre['bottom']) == 0 and ele_now['color'] == ele_pre['color'] and ele_now[
		'font-size'] == ele_pre['font-size']:
		return True
	if ele_now['font-family'] == ele_pre['font-family'] and ele_now['color'] == ele_pre['color'] and ele_now[
		'font-size'] == ele_pre['font-size'] and (
						ele_now['left'] - ele_pre['left'] < 1 and ele_now['left'] - ele_pre['left'] > -50):
		return True
	return False
...
if __name__ == '__main__':
	...
	path = '/Users/caolei/PyWorkSpace/FliggyDataProcess/tripadvisor/oversea/ccdcd/'
	out_path = '/Users/caolei/PyWorkSpace/FliggyDataProcess/tripadvisor/oversea/ccdcd/'
